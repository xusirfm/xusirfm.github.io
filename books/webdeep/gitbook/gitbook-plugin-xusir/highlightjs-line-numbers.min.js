! function (n, e) {
    ...
    function t() {
        ...
        n.type = "text/css", n.innerHTML = u(".{0}{border-collapse:collapse}.{0} td{padding:0}.{1}:before{content:attr({2})}", [f, m, j]), e.getElementsByTagName("head")[0].appendChild(n)
    }

    function r(t) {
        "interactive" === e.readyState || "complete" === e.readyState ? l(t) : n.addEventListener("DOMContentLoaded", function () {
            l(t)
        })
    }
    ...
    function o(n, e) {
        ...
        if ("" === t[t.length - 1].trim() && t.pop(), t.length > e) {
            for (var r = "", l = 0, i = t.length; l < i; l++) r += u('<li><span class="{0}"><span class="{1} {2}" {3}="{5}"></span></span><span class="{4}"><span class="{1}">{6}</span></span></li>', [p, v, m, j, g, l + 1, t[l].length > 0 ? t[l] : " "]);
            return u('<ul class="{0}">{1}</ul>', [f, r])
        }
        return n
    }
    ...
    function a(n) {
        ...
        if (/hljs-/.test(e)) {
            for (var t = c(n.innerHTML), r = 0, l = ""; r < t.length; r++) {
                var i = t[r].length > 0 ? t[r] : " ";
                l += u('<span class="{0}">{1}</span>\n', [e, i])
            }
            n.innerHTML = l.trim()
        }
    }
